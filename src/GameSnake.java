...
public class GameSnake {
    ...
    final int START_LOCATION = 200; // левый верхний угол окна,стартовая позиция
    final int START_SNAKE_SIZE = 6;
    final int START_SNAKE_X = 10; // стартовые координаты
    final int START_SNAKE_Y = 10;
    final int SHOW_DELAY = 150; // задержка(нужна при движении,иначе все будет очень быстро)
    final int LEFT = 37; // это коды клавиш,их получает обработчик
    final int UP = 38;
    final int RIGHT = 39;
    final int DOWN = 40;
    ...
    public static void main(String[] args) {
        new GameSnake().go(); //создаем объект и запускаем его метод go
    }

    void go() {
        frame = new JFrame(TITLE_OF_PROGRAM + " : " + START_SNAKE_SIZE);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(FIELD_WIDTH * POINT_RADIUS + FIELD_DX, FIELD_HEIGHT * POINT_RADIUS + FIELD_DY);//размеры окна
